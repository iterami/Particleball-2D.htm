function draw(){
        ...
        do{
            ...
            if(particles[i][1]+2 > players[0][8] || particles[i][1]-2 < players[1][8] + players[1][10]){
                ...
                if(j >= 0){
                    do{
                        /*x collisions*/
                        if(particles[i][0] >= obstacles[j][0] && particles[i][0] <= obstacles[j][0] + obstacles[j][2]){
                            if(particles[i][3] > 0){
                                if(particles[i][1] > obstacles[j][1] - 2 && particles[i][1] < obstacles[j][1]){
                                    particles[i][3] *= -1
                                }
                            }else if(particles[i][1] > obstacles[j][1] + obstacles[j][3] && particles[i][1] < obstacles[j][1] + obstacles[j][3] + 2){
                                particles[i][3] *= -1
                            }

                        /*y collisions*/
                        }else if(particles[i][1] >= obstacles[j][1] && particles[i][1] <= obstacles[j][1] + obstacles[j][3]){
                            if(particles[i][2] > 0){
                                if(particles[i][0] > obstacles[j][0] - 2 && particles[i][0] < obstacles[j][0]){
                                    particles[i][2] *= -1
                                }
                            }else if(particles[i][0] > obstacles[j][0] + obstacles[j][2] && particles[i][0] < obstacles[j][0] + obstacles[j][2] + 2){
                                particles[i][2] *= -1
                            }
                        }
                    }while(j--)
                ...
                if(particles[i][1] > players[1][1] + players[1][3] && particles[i][1] < players[0][1]){
                    if(particles[i][0] > -88 && particles[i][0] < 88){
                        if(particles[i][1] > 0){
                            if(particles[i][0] > players[0][0] - 2 && particles[i][0] < players[0][0] + players[0][2] + 2){
    ...
    if(i >= 0){
        do{
            ...
            buffer.fillStyle = 'rgb(' + players[i][4] + ',' + players[i][5] + ',' + players[i][6] + ')';
...
function save(){
    ...
    do{
        ...
        if(isNaN(get(j).value) || get(j).value === [1,10,3,25,100,200,420,1.5,65][i]){
            ...
            settings[i] = [1,10,3,25,100,200,420,1.5,65][i];
            get(j).value = settings[i]
        }else{
            settings[i] = parseFloat(get(j).value);
            ls.setItem('particleball' + i,settings[i])
        }
    }while(i--);
    ...
    if(settings[9]){
        ls.removeItem('particleball9')
    }else{
        ls.setItem('particleball9',0)
    }
    ...
    do{
        if(get(['move-keys','restart-key'][i]).value === ['AD','H'][i]){
            ...
            settings[i + 10] = ['AD','H'][i]
        }else{
            settings[i + 10] = get(['move-keys','restart-key'][i]).value;
            ls.setItem('particleball' + (i + 10),settings[i + 10])
        }
    }while(i--)
}
